		...
		return (root, query, builder) -> {
			...
			if (hasText(filter.actor())) {
				...
				predicate = builder.and(predicate, builder.or(
						builder.like(builder.lower(root.get("userEmail")), value),
						builder.like(builder.lower(root.get("userId")), value)));
			}
			if (hasText(filter.target())) {
				...
				predicate = builder.and(predicate, builder.or(
						builder.like(builder.lower(root.get("userEmail")), value),
						builder.like(builder.lower(root.get("userId")), value),
						builder.like(builder.lower(root.get("entityId")), value)));
			}
			if (hasText(filter.eventType())) {
				predicate = builder.and(predicate,
						builder.like(builder.lower(root.get("eventType")), contains(filter.eventType())));
			}
			if (hasText(filter.serviceName())) {
				predicate = builder.and(predicate,
						builder.like(builder.lower(root.get("serviceName")), contains(filter.serviceName())));
			}
