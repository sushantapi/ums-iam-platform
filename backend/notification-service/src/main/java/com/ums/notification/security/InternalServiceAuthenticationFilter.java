...
public class InternalServiceAuthenticationFilter extends OncePerRequestFilter {
	...
	public static final String INTERNAL_SERVICE_SECRET_HEADER = "X-Internal-Service-Secret";
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if (hasValidInternalSecret(request)) {
			UsernamePasswordAuthenticationToken authentication = new UsernamePasswordAuthenticationToken(
					"internal-service", null, List.of(new SimpleGrantedAuthority(INTERNAL_SERVICE_ROLE)));
	...
	private boolean hasValidInternalSecret(HttpServletRequest request) {
		...
		return StringUtils.hasText(internalServiceSecret) && internalServiceSecret.equals(suppliedSecret);
	}
}
